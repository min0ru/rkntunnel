...
cat domains/* | xargs -n1 -P16 dig +short | egrep -v "\.$" | sort -u >> /tmp/final_ip_list.txt

echo "Resolving custom domains second time with google DNS"
cat domains/* | xargs -n1 -P16 dig @8.8.8.8 +short | egrep -v "\.$" | sort -u >> /tmp/final_ip_list.txt

echo "Resolving custom domains second time with cloudflare DNS"
cat domains/* | xargs -n1 -P16 dig @1.1.1.1 +short | egrep -v "\.$" | sort -u >> /tmp/final_ip_list.txt

echo "Resolving custom domains third time with yandex DNS"
cat domains/* | xargs -n1 -P16 dig @77.88.8.1 +short | egrep -v "\.$" | sort -u >> /tmp/final_ip_list.txt
